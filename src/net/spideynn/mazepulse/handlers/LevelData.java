...
public class LevelData {
	...
	public static void init() {
		...
		try {
			SC_FONT = Font.createFont(
					Font.TRUETYPE_FONT,
					LevelData.class.getResourceAsStream("/fonts/SECRCODE.TTF"));
		...
		goalList = new HashMap<>();
		limitList = new HashMap<>();
		bouncerList = new HashMap<>();
		holeList = new HashMap<>();
		spawnerList = new HashMap<>();
		...
		spawners.add(new Spawner(320, 240, 100, 100, 120, 1.4));
	...
	public static ArrayList<Bouncer> getBouncers() {
		if(bouncerList.get(currentLevel) == null) return new ArrayList<>();
		return bouncerList.get(currentLevel);
	}
	
	public static ArrayList<Hole> getHoles() {
		if(holeList.get(currentLevel) == null) return new ArrayList<>();
		return holeList.get(currentLevel);
	}
	
	public static ArrayList<Spawner> getSpawners() {
		if(spawnerList.get(currentLevel) == null) return new ArrayList<>();
